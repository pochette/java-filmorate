...
public class UserService {
  ...
  @Autowired private final List<UserPredicate> validators;
  ...
  public void addFriend(Long userId, Long friendId) {
    Collection<User> usersFriend = userStorage.getFriendsByUserId(userId);
  ...
  public void deleteUser(Long id) {
    userStorage
        .getUserById(id)
        .orElseThrow(
            () -> new UserNotFoundException("Пользователь для удаления не найден ID= " + id));
  ...
  public void removeFromFriend(Long userId, Long friendId) {
    ...
    userStorage
        ...
        .orElseThrow(() -> new UserNotFoundException("Пользователь " + friendId + " не найден"));

    boolean isFriend =
        userStorage.getFriendsByUserId(userId).stream()
